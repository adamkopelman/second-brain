# Dashboard QuickAdd Actions — Design
...
but gives no way to *act* — adding a task (next action) or a new project still requires leaving
Obsidian for a Claude Code slash command. The user wants the dashboard to be "the frontend of the
repo": one screen where they can both see everything and add tasks/projects directly.

## Goals

1. Make the dashboard **actionable**: clickable links that capture a task, add a next action to a
   project, or create a new project — without leaving Obsidian.
2. Keep the existing KPI/card-grid behavior intact — this is additive, not a rewrite.
3. Keep the vault self-contained (vendor the one new plugin needed, same as Dataview/Claudian
...
- No "Mission" or "Areas of Responsibility / Pillars" concept — out of scope, not what was asked.
...
| **New Project** | Template | Prompts for title, then an area suggester listing files in `20 Areas/`. Creates `10 Projects/{title}.md` from `_templates/Project.md`, filling `created` = today, `review` = today + 7 days, `area` = the chosen area (wikilinked, blank if none picked). |
...
### `Dashboard.md` layout changes

Additive only — the existing `dataviewjs` KPI/card-grid block is untouched. New content: an
"➕ Actions" row directly under the KPI tiles, before the "Next actions by context" card — three
markdown links (Quick Capture / New Next Action / New Project) using the URI scheme above.

`Home.canvas` — the "⚡ Capture & Engage" text node gains a line naming the three new dashboard
actions, so the launchpad documents them too.

### Mirroring & docs

- Updated `Dashboard.md` and `Home.canvas` get copied into
